	...
	Draggable.prototype.onDragEnd = function( instance, event, pointer ) {
		...
		for( var i = 0, len = this.droppables.length; i < len; ++i ) {
			...
			if( droppableEl.isDroppable( instance.element ) ) {
				...
				var elementIns = instance.element;

				
				var dupe = false;
				$('#dropItems #boxed').each(function(){
					elementsIn.push($(this).text()); 
				});

				for (var i = 0; i < elementsIn.length; i++) {
					

					if ( $(elementIns).html().toString() == elementsIn[i]  ) {
						dupe = true;
					};
				};

				if (!dupe) {
					$('#dropItems').append('<div id="boxed" class="selectable">' + $(elementIns).html() + '</div>');
					$('#swag').append($(elementIns).html());
				};
